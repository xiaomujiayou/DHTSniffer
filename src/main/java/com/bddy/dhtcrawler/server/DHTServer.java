package com.bddy.dhtcrawler.server;
...
import com.bddy.dhtcrawler.structure.Node;
...
public class DHTServer extends Thread {
    ...
    public DHTServer(String hostname, int port, int maxGoodNodeCount) {
    	...
    	try {
			ls = new LookupService(System.getProperty("user.dir")+"/GeoIP.dat",LookupService.GEOIP_MEMORY_CACHE);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private byte[] getNeighbor(byte[] info_hash) {
    	...
    	System.arraycopy(info_hash, 0, bytes, 0, 8);
    	System.arraycopy(getNodeId(), 8, bytes, 8, 12);
    	return bytes;
    }
    ...
    private static byte[] createRandomNodeId() {
