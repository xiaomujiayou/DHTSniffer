...
import com.xm.dht.listener.OnMetadataListener;
import com.xm.dht.main.Main;
import com.xm.dht.structure.Info;
import com.xm.dht.structure.SubFile;
import com.xm.dht.structure.Torrent;
import com.xm.dht.util.ByteUtil;
import com.xm.dht.util.NodeIdUtil;
...
public class AnnouncePeerInfoHashWireHandler implements IInfoHashHandler {
	...
	private Torrent parseTorrent(Map<String, Object> map) throws UnsupportedEncodingException {
		...
		if (map.containsKey("announce")) {
			torrent.setAnnounce(new String((byte[]) map.get("announce"), encoding));
		}
		
		if (map.containsKey("comment")) {
			torrent.setComment(new String((byte[]) map.get("comment"), encoding).substring(0, 300));
		}
		
		if (map.containsKey("created by")) {
			torrent.setCreatedBy(new String((byte[]) map.get("created by"), encoding));
		}
		...
		if (info.containsKey("files")) {
			...
			for (Map<String, Object> f : list) {
				...
				if (f.containsKey("path.utf-8")) {
					subFile = new SubFile(length, new String(((List<byte[]>) f.get("path")).get(0), encoding));
				} else {
					subFile = new SubFile(length, new String(((List<byte[]>) f.get("path")).get(0), encoding));
				}
	...
	private void exit() {
		...
		if (onMetadataListener != null)
			onMetadataListener.onMetadata(torrent,address.getAddress().getHostAddress());
	}
